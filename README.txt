PROJECT : Tic Tac Toe Game

DESCRIPTION : 
    1. This project is a classic Tic Tac Toe game.
    2. It is a single-player game (Human vs. Computer).
    3. User and Computer will play one turn at a time and they play one after the other.
    4. The user starts the game.
    5. User plays with symbol X.
       Computer plays with symbol O.
    6. A player (user or computer) wins if he/she has 3 symbols (all X or all O respectively) in vertical, 
       diagonal or horizontal order.
    7. The game has three levels:
        1. Easy
        2. Medium
        3. Hard
       according to difficulty.
    8. The Hard level is undefeatable.
    9. The Minimax algorithm was used for Artificial Intelligence (AI) in the Medium and Hard levels.
    10. To switch between levels, click on any one of 'EASY', 'MEDIUM' or 'HARD' buttons.
    11. If any one of the 3 buttons described above is pressed in the middle of a game, the board is refreshed for the selected level.

LANGUAGE :
    The program is written in Python and requires a Python interpreter for compilation and successful execution.

PLATFORM :
    Mainly targets Linux distributions and Windows. This application was developed on Ubuntu 14.04.

REQUIREMENTS:
    1. Linux Platforms:
        The game requires the TkInter Python module for successful execution.
        To install Tkinter module type the following command on the command prompt of your terminal.
	        $ sudo apt-get install python-tk

    2. Windows:
        Install Python (version 2.x) on your system.

EXECUTION:
    1. Linux Platforms:
        Execute the ttt.py file to run the game. To start the application type the following command on the command
        prompt of your terminal.
            $ python ttt.py
        [Make sure that you are in the folder consisting of the ttt.py file before executing.]
    
    2. Windows:
        Double click on the ttt file.
 			
DEVELOPED BY:
    1. Aniket Kulkarni     (111403064)   kaniket21@gmail.com
    2. Rohan Patil         (111408046)   rohanrahulpatil89@gmail.com
    3. Sumedh Kale         (111403062)   sumedh_kale@yahoo.com

View this project on GitHub at https://github.com/aniketk21/tictactoe
